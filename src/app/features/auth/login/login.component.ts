...
@Component({
  ...
  template: `
    <div class="min-h-screen flex flex-col lg:flex-row" style="background:#0E0608">
      ...
      <div class="relative lg:w-1/2 lg:min-h-screen overflow-hidden"
        ...
        <!-- Overlay lateral derecho (solo desktop) -->
        ...
        <div class="relative z-10 h-full flex flex-col justify-end p-6 lg:p-10 pb-8 lg:pb-12">
          ...
          <h2 class="text-2xl lg:text-3xl font-black text-white leading-tight mb-2">
            La vinotinto no clasificó.<br/>
            <span style="color:#C9A843">Pero Tu pronósticos si!.</span>
          </h2>
          <p class="text-gray-100 text-sm lg:text-base">Tu puede ser el próximo técnico de la vinotinto.</p>

        </div>
      </div>

      <!-- ═══ FORMULARIO (derecha desktop / abajo mobile) ═══ -->
      <div class="flex-1 flex items-center justify-center px-6 py-10 lg:py-0">
        <div class="w-full max-w-sm animate-fade-in">
          ...
          <div *ngIf="error()" class="mb-5 p-3 rounded-xl text-sm text-red-400"
          ...
          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-5">
            ...
            <div>
              ...
              <p *ngIf="form.get('email')?.invalid && form.get('email')?.touched"
                 class="mt-1.5 text-xs text-red-400">Ingresá un email válido.</p>
            </div>
            ...
            <div class="flex items-center justify-between">
              ...
              <button type="button" (click)="forgotPassword()"
                class="text-sm font-semibold transition-colors hover:opacity-80" style="color:#C9A843">
                ¿Olvidaste tu contraseña?
              </button>
            </div>

            <button type="submit" [disabled]="loading() || form.invalid"
              class="w-full py-3.5 px-6 rounded-xl font-black uppercase tracking-wider text-sm transition-all hover:scale-[1.02] disabled:opacity-40 disabled:cursor-not-allowed"
          ...
          <button (click)="googleLogin()" [disabled]="loading()"
            ...
            style="background:transparent;border:1px solid #2A1219">
          ...
          <p class="text-center text-sm text-gray-400 mt-8">
            ¿No tenés cuenta?
            <a routerLink="/auth/registro" class="font-bold ml-1 hover:opacity-80 transition-opacity" style="color:#C9A843">
              Registrate gratis
            </a>
          </p>
        </div>
